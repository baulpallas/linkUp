...
router.post('/login', (req, res) => {
    ...
    if (!user) res.status(400).json({message: "User with that email does not exist.  Try another email."})
    if (user && user.password !== password) res.status(400).json({message: "Incorrect passowrd"})
...
router.post('/signup', (req, res) => {
    ...
    if (user) res.status(400).json({message: "User with that email already exists.  Try another email."})
